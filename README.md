...
class RootTopic extends Topic {
    ...
    async onReceive(context) {
        ...
        if (context.request.text === "book travel") {
            this.state.child = await new TravelTopic().createTopicInstance(
                context, async () => {
...
bot.onReceive(async context => {
    await Topic.do(context, () => new RootTopic().createTopicInstance(context));
})
```
...
### `Topic.createTopicInstance(context: BotContext, args?, returnToParent: (context: BotContext, args?) => Promise<void>): Promise<void>`

Because JavaScript does not support asynchronous constructors, topics must be created in a two step process:

1. call the Topic's constructor
2. call that instance's asynchronous `createTopicInstance()` method (which itself calls the `init()` topic method)

This is typically done in a single line:

```ts
    const instance = new MyTopic().createTopicInstance(context);
```

If the topic's `init()` method calls `returnToParent()`, `createTopicInstance()` will return `undefined`.
...
```ts
bot.onReceive(async context => {
    await Topic.do(context, () => new rootTopicClass.createTopicInstance(context));
})
```
...
class MyTopic extends Topic<State> {
...
class YourTopic extends Topic<YourState>('yourTopic')
    async init(context: BotContext) {
        this.state.child = await new MyTopic().createTopicInstance(
            context, {
                bar: "hey"                           // error
            }, args => {
...
const myTopicClass = new TopicClass<InitArgs, State, ReturnArgs>('myTopic')
...
const yourTopicClass = new TopicClass<undefined, YourState, undefined>('yourTopic')
    .init(async (context, topicContext) => {
        topicContext.instance.state.child = await topicContext.createTopicInstance(myTopicClass, {
            bar: "hey"                              // error
        });
    })
    .onReceive(async (context, topicContext) => {
