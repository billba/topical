...
2. call that instance's asynchronous `createTopicInstance()` method (which itself calls the private `init()` method)
...
If the `init()` method calls `returnToParent()`, `createTopicInstance()` will return `undefined`.
